...
const UserHeader = () => {
  ...
  const handleImageUpload = async (event) => {
    ...
    try {
      const response = await fetch(
        "https://api.cloudinary.com/v1_1/da5ptw96m/image/upload",
        {
          method: "POST",
          body: formData,
        }
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      try {
        const response = await fetch("https://my-eventproposalpage.onrender.com/api/user/profile", {
          headers: {
  ...
  <AiFillApple size="20%" color="#64A1F5"/> Logo
      ...
      <section className="vender-box">
        ...
        <CloudinaryContext cloudName="da5ptw96m">
          {selectedImage ? (
            <Image
